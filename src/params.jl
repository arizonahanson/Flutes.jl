
# ~~~~~ legend ~~~~~
ℓ # absolute distance from embouchure center (Z axis)
⌀ # inner diameter (XY plane)
ℎ # height/depth (Y axis)
𝑑 # diameter (Z axis in XZ plane)
𝜙 # ellipse width (X axis in XZ plane)
𝜃 # undercut angle (∠ °XY plane)
# ~~ subscripts ~~
ᵩ # effective length
₀ # crown (z=0)
ᵣ # reflector
ₑ # embouchure
ₚ # lip-plate
ₔ # bore taper end
ₙ # tenon headjoint-body
ₐ # headjoint end
ₕ # tone hole
ₘ # mortise body-foot
ₜ # flute end
ₓ # extruder

# ----- headjoint -----
𝛥ℓᵩ # correction at embouchure    52.0mm
⌀₀ # crown diameter               24.0mm
ℓ₀ # embouchure-crown distance    30.0mm
⌀ᵣ # bore diameter at reflector   17.0mm
ℓᵣ # reflector-embouchure length  17.0mm
⌀ₑ # bore diameter at embouchure  17.4mm
𝑑ₑ # embouchure hole diameter     12.0mm
𝜙ₑ # embouchure hole width        10.0mm (𝜙ₑ/𝑑ₑ)
𝜃ₑ # embouchure undercut angle    11.7°
ℎₑ # embouchure hole height        4.3mm
𝑑ₚ # lip-plate length             50.0mm
𝜙ₚ # lip-plate width              26.0mm
ℓₔ # embouchure-taper distance   120.0mm
⌀ₔ # bore diameter at taper stop  19.0mm
ℎₔ # flute wall depth @ taper stop 2.5mm
ℓₙ # tenon start                 126.0mm
ℎₙ # tenon wall depth             1.25mm
ℓₐ # embouchure-headjoint length 156.0mm

# ----- flute -----
𝜗  # bore air temperature         25.0°C
𝑓ₜ # fundamental frequency
ℓₜ # bore length

# ----- per hole -----
𝑔  # interval ratio minus one     2^(1/12)-1
𝑓ₕ # frequency of hole
⌀ₕ # bore diameter at tone hole   19.0mm
ℎₕ # tone hole height             2.5mm
𝑑ₕ # tone hole diameter           ~~
𝜃ₕ # tone hole undercut           ~~
ℓₕ # embouchure-hole distance     ~~

# ---- printing -----
⌀ₓ # extruder bore diameter       0.4mm

